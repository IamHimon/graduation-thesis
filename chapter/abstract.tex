...
(1)本文着重研究了文本记录分割与命名属性值识别问题，介绍了已有的解决方法，描述了具有代表性的各类算法的优势，并分析了其中存在的问题。

(2)提出了一种新颖的文本记录分割与命名属性值识别算法。本算法基于卷积神经网络，提出了一种贪婪式的概率匹配算法来做全局标注和分割，在抽取过程中学习出一个双向的序列与位置模型来修正一些错误的结果。这一算法有效的解决了传统方法中的各种问题，比传统的方法的抽取质量提高了超过10\%，并在抽取效率上也有非常好的表现。
...
and Text Segmentation. Text Record Segmentation and Named Attribute Values Recognition(TesaRec), also called Information Extraction by Text Segmentation (IETS) in ~\cite{cortez2010ondux}, is the problem of segmenting unstructured textual inputs to extract implicit data values contained in them.
...
%
\begin{enumerate}[(1)]
\item We focus on the problem of TesaRec in the paper. Some existing TesaRec methods are introduced here and the advantages and disadvantages of them are also analysed.
